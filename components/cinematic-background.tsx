...
export function CinematicBackground() {
  ...
  const orbAY = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : -480]);
  const orbAX = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : 220]);

  const orbBY = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : 380]);
  const orbBX = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : -260]);

  const orbCY = useTransform(scrollYProgress, [0, 1], [0, reduceMotion ? 0 : -260]);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="absolute -left-40 top-[10%] h-[520px] w-[520px] rounded-full bg-cyan-400/25 blur-[140px]"
      />
      <motion.div
        style={{ y: orbBY, x: orbBX }}
        className="absolute -right-40 top-[40%] h-[620px] w-[620px] rounded-full bg-indigo-400/25 blur-[160px]"
      />
      <motion.div
